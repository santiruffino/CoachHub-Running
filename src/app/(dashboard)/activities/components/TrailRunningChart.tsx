...
interface TrailRunningChartProps {
    activityId: string;
    laps?: Lap[];
    hrZones?: HRZone[];
}
...
export function TrailRunningChart({ activityId, laps, hrZones }: TrailRunningChartProps) {
    ...
    const option = useMemo(() => {
        ...
        const paceSeries: Record<string, unknown> = {
            ...
            areaStyle: {
                color: {
                    ...
                    x: 0, y: 0, x2: 0, y2: 1,
                    colorStops: [
                        { offset: 0, color: 'rgba(6, 71, 212, 0.15)' },
                        { offset: 1, color: 'rgba(6, 71, 212, 0)' }
                    ]
                }
            }
        };
        ...
        if (hasElevationData) {
            const validElevations = chartData.series.elevation.filter((e): e is number => e !== null);
            ...
            const elevationYAxis: Record<string, unknown> = {
                type: 'value',
                name: 'm',
                position: 'right',
                show: legendSelected[elevationMetricName],
                min: Math.floor(minElev - 10),
                max: Math.ceil(maxElev + 10),
                axisLabel: {
                    formatter: '{value}m',
                    color: '#8B5CF6',
                },
            ...
            const elevationSeries: Record<string, unknown> = {
                name: elevationMetricName,
                type: 'line',
                yAxisIndex: yAxisIndex,
            ...
            yAxisArray.push(elevationYAxis);
            seriesArray.push(elevationSeries);
            yAxisIndex++;
        }
                ...
                formatter: (params: TooltipParam[]) => {
                    ...
                    params.forEach((param) => {
                        ...
                        if ((param.seriesName === paceMetricName || param.seriesName === gapMetricName) && param.value) {
                            ...
                            tooltipText += `${param.marker} ${param.seriesName}: ${param.value.toFixed(0)} bpm<br/>`;
                        }
                    });
                    return tooltipText;
                }
            ...
            <Card className="w-full h-full bg-endurix-paper dark:bg-card border border-endurix-black/10 dark:border-border flex flex-col justify-between p-6 min-h-[460px] lg:min-h-[560px]">
                ...
                <div className="w-full flex-1 min-h-[260px] relative mt-4 overflow-hidden">
                    ...
                    >
                        ...
                        />
                        <path
                            d="M 0 180 Q 80 170 160 175 T 320 165 T 500 170 L 500 200 L 0 200 Z"
                            fill="currentColor"
                            opacity="0.1"
                        />
            ...
            <CardHeader className="border-b border-endurix-black/10 dark:border-border">
                <div className="flex items-start justify-between flex-wrap gap-4">
                    ...
                    <div className="flex flex-wrap gap-3 items-center">
                        <div className="flex items-center gap-2">
                            <span className="text-[10px] uppercase tracking-widest text-endurix-black/50 dark:text-muted-foreground" style={{ fontFamily: 'var(--font-ibm-plex-mono, monospace)' }}>{t('xAxis')}:</span>
                        ...
                        {streams && (
                            <div className="flex items-center gap-2">
                                <span className="text-[10px] uppercase tracking-widest text-endurix-black/50 dark:text-muted-foreground" style={{ fontFamily: 'var(--font-ibm-plex-mono, monospace)' }}>{t('detail')}:</span>
                        ...
                        {laps && laps.length > 0 && (
                            <div className="flex items-center gap-2">
                                <span className="text-[10px] uppercase tracking-widest text-endurix-black/50 dark:text-muted-foreground" style={{ fontFamily: 'var(--font-ibm-plex-mono, monospace)' }}>{t('laps')}:</span>
