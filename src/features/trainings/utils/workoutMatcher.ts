 ...
 *
 * Order of precedence:
 *  1. `block.rpe` — the explicit RPE slider value.
 *  2. `block.target` when it's an `rpe_target` — the RPE the coach set as the
 *     block objective (stored in target.min/max, not in `block.rpe`).
 *
 * We deliberately do NOT fall back to `block.intensity`: that field is the
 * graph bar height (0-100), not an effort level. Conflating the two made an
 * easy run (intensity 85) show up as "RPE 9".
 */
export function deriveStepRpe(block: RawBlock): number | undefined {
...
function matchContinuousEffortSegments(segments: ContinuousEffortSegment[], flatSteps: FlatStep[]): MatchedLap[] {
    ...
    while (segmentIndex < segments.length) {
        ...
        const { matches, variance, confidence } = isMatch(segmentValue, stepValue, targetType, step.stepType);
        ...
        const tolerance = getStepTolerance(step.stepType);
        ...
        for (let j = segmentIndex; j < segments.length; j++) {
            ...
            const cumulativeMatch = isMatch(cumulativeValue, stepValue, targetType, step.stepType);
        ...
        for (let lookAhead = 1; lookAhead <= maxLookAhead && currentStepIndex + lookAhead < flatSteps.length; lookAhead++) {
            ...
            const lookAheadResult = isMatch(nextSegmentValue, nextStep.target_value, nextTargetType, nextStep.stepType);
...
function isMatch(
    ...
    const tolerance = getStepTolerance(stepType);
...
export function matchLapsToWorkout(
    ...
    while (i < laps.length) {
        ...
        if (step.target_type === 'distance') {
            lapValue = getLapValueByTargetType(lap, 'distance'); // in meters
            stepValue = step.target_value;
            targetType = 'distance';
            debugLog('Comparing DISTANCE:', { lapValue, stepValue });
        } else if (step.target_type === 'duration') {
            lapValue = getLapValueByTargetType(lap, 'duration'); // in seconds
            stepValue = step.target_value;
            targetType = 'duration';
        ...
        if (matches) {
            ...
            if (cumulativeEndIndex >= i) {
                debugLog('CUMULATIVE MATCH found:', {
                    startLap: i,
                    endLap: cumulativeEndIndex,
                    stepIndex: currentStepIndex,
                    cumulativeValue,
                    stepValue,
                    variance: cumulativeVariance.toFixed(2) + '%',
                });
    ...
    debugLog('No direct lap matches found, falling back to continuous effort segments:', segments);
